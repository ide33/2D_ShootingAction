...
[CreateAssetMenu(fileName = "CannonBullet", menuName = "ShotStructure/CannonBullet")]
public class CannonBulletShotStructure : BulletShotStructure
{
    ...
    private void FireProjectile(Transform firingPoint, float firingAngle, float forceMagnitude, Vector2 Turndirection)
    {
        ...
        SpriteRenderer spriteRenderer = newCannonAmmo.GetComponent<SpriteRenderer>();
        ...
        Ammo ammo = newCannonAmmo.GetComponent<Ammo>();
        ...
        if (ammo != null)
        {
            // 弾クラスのUpdete処理に移動方向に回転する処理をデリゲートとして追加
            ammo.OnUpdateMoveAction = deltaTIme =>
            {
                ...
                if (velocity.sqrMagnitude > 0.01f) // ほぼ静止状態でなければ回転
                {
                    ...
                    newCannonAmmo.transform.rotation = Quaternion.Euler(0, 0, angle);
                }
            };
        }
